...
export class VertexImageService {
  ...
  async generateSlideImage(
    ...
    try {
      console.log(`🎨 Generating image for slide ${slideData.slideNumber}/${slideData.totalSlides}`);

      // Build the image generation prompt
      const prompt = this.buildSlideImagePrompt(slideData, templateStyle, presentationStyle, targetAudience);
      ...
      console.error(`❌ Error generating slide image for slide ${slideData.slideNumber}:`, error);
      return await this.generateFallbackSlideImage(slideData, templateStyle);
    }
  }
  ...
  private buildSlideImagePrompt(
    ...
    const templateStyles: Record<string, string> = {
      modern: 'Clean minimalist design with gradient backgrounds, subtle geometric shapes, modern sans-serif typography',
      classic: 'Traditional professional design with solid colors, classic serif fonts, formal layout',
      creative: 'Bold vibrant colors, dynamic shapes, artistic elements, creative typography',
      professional: 'Corporate sleek design, dark themes, subtle accents, professional icons',
      educational: 'Friendly engaging design, bright colors, illustrated elements, approachable style',
      minimal: 'Ultra-clean design, lots of whitespace, simple geometric accents, elegant typography'
    };

    // Define audience-appropriate visual style
    const audienceStyles: Record<string, string> = {
      school: 'colorful, fun illustrations, simple icons, engaging visuals for young learners',
      college: 'academic diagrams, infographics, balanced visuals for university students',
      professional: 'data visualizations, charts, corporate imagery, business-appropriate',
      training: 'step-by-step visuals, instructional graphics, hands-on demonstration style'
    };

    const styleDescription = templateStyles[templateStyle] || templateStyles.modern;
    const audienceDescription = audienceStyles[targetAudience] || audienceStyles.college;

    // Use custom image prompt if provided, otherwise create one
    const visualContext = slideData.imagePrompt ||
...
The image should be a complete presentation slide ready to display, with clear visual hierarchy and professional design.`;
  ...
  private async generateFallbackSlideImage(
    ...
    const colorSchemes: Record<string, { primary: string; secondary: string; bg: string; text: string; accent: string }> = {
      modern: { primary: '#F97316', secondary: '#FDBA74', bg: '#FFF7ED', text: '#1F2937', accent: '#EA580C' },
      classic: { primary: '#3B82F6', secondary: '#93C5FD', bg: '#EFF6FF', text: '#1E3A8A', accent: '#2563EB' },
      creative: { primary: '#A855F7', secondary: '#E879F9', bg: '#FAF5FF', text: '#581C87', accent: '#9333EA' },
      professional: { primary: '#374151', secondary: '#6B7280', bg: '#F3F4F6', text: '#111827', accent: '#4B5563' },
      educational: { primary: '#10B981', secondary: '#6EE7B7', bg: '#ECFDF5', text: '#064E3B', accent: '#059669' },
      minimal: { primary: '#64748B', secondary: '#94A3B8', bg: '#F8FAFC', text: '#1E293B', accent: '#475569' }
    };
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="1920" height="1080">
  ...
  <text x="120" y="170" font-family="system-ui, -apple-system, sans-serif" font-size="64" font-weight="700" fill="${colors.text}">
    ${this.escapeXml(slideData.title.substring(0, 50))}${slideData.title.length > 50 ? '...' : ''}
  </text>
  ...
  <g transform="translate(120, 280)">
    ...
    <g transform="translate(0, ${index * 120})">
      ...
      <text x="60" y="40" font-family="system-ui, -apple-system, sans-serif" font-size="36" fill="${colors.text}">
        ${this.escapeXml(point.substring(0, 80))}${point.length > 80 ? '...' : ''}
      </text>
    </g>
    `).join('')}
  </g>
  ...
  <rect x="1200" y="300" width="600" height="500" rx="20" fill="${colors.secondary}" opacity="0.3"/>
  <text x="1500" y="550" font-family="system-ui, sans-serif" font-size="24" fill="${colors.text}" opacity="0.6" text-anchor="middle">
    Visual Content
  </text>
  <text x="1500" y="580" font-family="system-ui, sans-serif" font-size="18" fill="${colors.text}" opacity="0.4" text-anchor="middle">
    Illustration Area
  </text>
  
  <!-- Footer -->
  <rect x="0" y="1040" width="1920" height="40" fill="${colors.primary}" opacity="0.1"/>
  <text x="1840" y="1060" font-family="system-ui, sans-serif" font-size="24" fill="${colors.text}" opacity="0.6" text-anchor="end">
  ...
  async generateAllSlideImages(
    ...
    const imageMap = new Map<string, { imageBase64: string; imageUrl: string }>();
