...
Enhanced prompt:`
  ...
  async generateImage(params: ImageGenerationParams): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error?.message || `Image generation failed: ${response.statusText}`
        );
      }
   ...
   */
  private createFallbackPrompt(query: string, category: string, language: string): string {
  ...
  async generateMultiplePosters(
    ...
    // Enhance once
    const enhancedPrompt = await this.enhanceQuery({ query, category, language });
    ...
    for (let i = 0; i < count; i += 2) {
      ...
      const promises = Array(batchSize).fill(null).map(() =>
        this.generateImage({ prompt: enhancedPrompt })
      );
