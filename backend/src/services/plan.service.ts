  ...
  async optimizeStudyPrompt(
    ...
    try {
      ...
      const prompt = `You are a Study Coach. 
            ...
            2. Format: "Explain ${topic} to me like I'm in grade ${
        grade || "12"
      }. Focus on [keyword 1] and [keyword 2]. Provide a real-world example."
            3. STRICT LIMIT: Maximum 4 lines of text.
            4. Style: Direct student-to-teacher query.`;

      const result = await ollamaChatService.generateWithMaxOutput(prompt, 200);
      ...
      return (
        cleaned.trim() ||
        `Explain the concept of ${topic} for a grade ${
          grade || "12"
        } student using details from the document.`
      );
    } catch (error: any) {
