...
export class WeaveService {
  ...
  async generatePresentation(
    ...
    try {
      ...
      console.log(
        `✅ Generated ${slidesWithImages.length} slides with images for presentation`
      )
  ...
  private generateFallbackSlides(request: PresentationRequest): Slide[] {
    ...
    for (let i = 0; i < request.numSlides; i++) {
      slides.push({
        ...
        content: `Content for slide ${i + 1} about ${
          request.topic
        }. This is placeholder content that would normally be generated by AI based on your presentation requirements.`,
        speakerNotes: `Speaker notes for slide ${
          i + 1
        }. Provide additional context and talking points related to the slide content.`,
        position: i + 1,
      })
    }
